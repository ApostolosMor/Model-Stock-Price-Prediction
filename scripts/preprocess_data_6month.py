...
def engineer_features_6m(df):
    ...
    df[TARGET_NAME] = (df['future_return_6m'] > 0).astype(int)
...
def clean_winsorize_scale(df):
    feature_cols = [c for c in df.columns if c not in ['Date','Ticker',TARGET_NAME,'future_price','future_return_6m']]
...
def split_and_save(df, features):
    ...
    test.to_csv(f'{PROCESSED_DIR}/test_6m.csv', index=False)
